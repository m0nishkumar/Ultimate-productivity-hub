...
export function ModelPorshe(props) {
  const { nodes, materials } = useGLTF("/free_porsche_911_carrera_4s.glb");
  const car=useRef()
  useFrame((state, delta) =>{ 
    car.current.rotation.y+=0.005;
    easing.dampC(materials.paint.color,props.color,1,delta);
    // easing.dampC(materials.silver.color,"red",1,delta);
  });
  useFrame((state,delta)=>{
    car.current.rotation.y+=0.002
})
  return (
    <group {...props} dispose={null} ref={car} position={[0,-0.5 , 0]}>
      <group
